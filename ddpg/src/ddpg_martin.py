...
class MartinDDPG:
    ...
    def __call__(self, env):
        ...
        buildActorModel = BuildActorModel(stateDim, actionDim, actionBound,
                                          self.fixedParameters['actorHiddenLayersWeightInit'],
                                          self.fixedParameters['actorHiddenLayersBiasInit'],
                                          self.fixedParameters['actorOutputWeightInit'],
                                          self.fixedParameters['actorOutputBiasInit'],
                                          self.fixedParameters['actorActivFunction'],
                                          self.fixedParameters['gradNormClipValue'],
                                          self.fixedParameters['normalizeEnv'])
        ...
        buildCriticModel = BuildCriticModel(stateDim, actionDim,
                                            self.fixedParameters['criticHiddenLayersWeightInit'],
                                            self.fixedParameters['criticHiddenLayersBiasInit'],
                                            self.fixedParameters['criticOutputWeightInit'],
                                            self.fixedParameters['criticOutputBiasInit'],
        ...
        getnoise = GetNoise(self.fixedParameters['varianceDiscount'], self.fixedParameters['minVar'],
                            self.fixedParameters['noiseDecayStartStep'], self.fixedParameters['noiseInitVariance'])
...
class BuildActorModel():
    def __init__(self, statedim, actiondim, actionbound, actorHiddenLayersInitinitweight, actorHiddenLayersinitbias,
                 actorOutputInitinitweight, actorOutputinitbias, actorActivFunction, gradNormClipValue, normalizeEnv):
        ...
        with graph.as_default():
            ...
            with tf.variable_scope('evalnet'):
                ...
                for unit in range(len(numberlayers)):
                    ...
                    activation_ = tf.layers.dense(activation_, units=numberlayers[unit],
                                                  kernel_initializer=self.actorHiddenLayersInitinitweight[unit],
                                                  activation=actorhiddenActivFunction,
                                                  bias_initializer=self.actorHiddenLayersinitbias[unit],
                                                  name="hidden" + str(unit + 1), trainable=True)
                ...
                Qevalvalue_ = tf.layers.dense(activation_, self.actionDim, actoroutputActivFunction,
                                              kernel_initializer=self.actorOutputInitinitweight,
            ...
            with tf.variable_scope('targetnet'):
                ...
                for unit in range(len(numberlayers)):
                    ...
                    activation_ = tf.layers.dense(activation_, units=numberlayers[unit],
                                                  ...
                                                  activation=actorhiddenActivFunction,
                                                  bias_initializer=self.actorHiddenLayersinitbias[unit],
                                                  name="hidden" + str(unit + 1), trainable=False)
                    if self.normalizeEnv == True:
                        activation_ = tf.layers.batch_normalization(activation_)
                actoroutputActivFunction = self.actorActivFunction[-1]
                Qnext = tf.layers.dense(activation_, self.actionDim, actoroutputActivFunction,
                                        kernel_initializer=self.actorOutputInitinitweight,
            ...
            with tf.name_scope("replaceParameters"):
                ...
                updateParam_ = [targetParams_[i].assign((1 - tau_) * targetParams_[i] + tau_ * evalParams_[i]) for i in
                                range(len(targetParams_))]
                ReplaceTargetParam_ = [tf.assign(targetParams_, evalParams_) for targetParams_, evalParams_ in
                                       zip(targetParams_, evalParams_)]
            ...
            with tf.variable_scope('Qaction'):
                ...
                policyGradient_ = tf.gradients(evalAction_, evalParams_,
                                               actionGradients_) if self.gradNormClipValue == None else [
                    tf.clip_by_norm(grad, self.gradNormClipValue) for grad in
                    tf.gradients(evalAction_, evalParams_, actionGradients_)]
...
class BuildCriticModel():
    def __init__(self, statedim, actiondim, criticHiddenLayersInitinitweight, criticHiddenLayersinitbias,
                 criticOutputInitinitweight, criticOutputinitbias, criticActivFunction, gradNormClipValue,
                 normalizeEnv):
        self.stateDim = statedim
        self.actionDim = actiondim
        ...
        with graph.as_default():
            ...
            with tf.variable_scope('evalnet'):
                ...
                for unit in range(len(numberlayers)):
                    ...
                    activation_ = tf.layers.dense(activation_, units=numberlayers[unit],
                                                  kernel_initializer=self.criticHiddenLayersInitinitweight[unit],
                                                  activation=critichiddenActivFunction,
                                                  bias_initializer=self.criticHiddenLayersinitbias[unit],
                                                  name="hidden" + str(unit + 1), trainable=True)
                    if self.normalizeEnv == True:
                        activation_ = tf.layers.batch_normalization(activation_)
                ...
                evalbias1 = tf.get_variable(name='evalbias1', shape=[numberlayers[-1]],
                                            initializer=self.criticHiddenLayersinitbias[-1])
                evalactivation = criticoutputActivFunction(
                    tf.matmul(activation_, evalstateweight_) + tf.matmul(action_, evalactionweight_) + evalbias1)
                Qevalvalue_ = tf.layers.dense(evalactivation, 1, activation=self.criticActivFunction[-1],
                                              kernel_initializer=self.criticOutputInitinitweight,
                                              bias_initializer=self.criticOutputinitbias, trainable=True)
            ...
            with tf.variable_scope('targetnet'):
                ...
                targetbias1 = tf.get_variable(name='targetbias1', shape=[numberlayers[-1]],
                                              initializer=self.criticHiddenLayersinitbias[-1])
                targetactivation = criticoutputActivFunction(
                    tf.matmul(activation_, targetstateweight_) + tf.matmul(targetaction,
                                                                           targetactionweight_) + targetbias1)
                Qnextvalue_ = tf.layers.dense(targetactivation, 1, activation=self.criticActivFunction[-1],
                                              kernel_initializer=self.criticOutputInitinitweight,
                                              bias_initializer=self.criticOutputinitbias, trainable=False)
...
def getQtarget(nextStatesBatch, targetactionsBatch, criticModel):
    ...
    Qtargetvalue_ = criticModel.run(Qnextvalue_,
                                    feed_dict={nextstates_: nextStatesBatch, targetaction_: targetactionsBatch})
...
class TrainCritic:
    ...
    def __call__(self, actormodel, criticmodel, minibatch, batchSize):
        ...
        criticmodel.run([trainopt_, loss_],
                        feed_dict={states_: statebatch, action_: actionbatch, nextstates_: nextStatebatch,
                                   reward_: rewardbatch,
